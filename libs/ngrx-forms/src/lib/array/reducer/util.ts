...
import { computeArrayState, FormArrayState, FormGroupControls, FormGroupState, FormState, isArrayState, isGroupState, KeyValue } from '../../state';
...
export function updateIdRecursive<TValue extends KeyValue = any>(state: FormState<TValue>, newId: string): FormState<TValue> {
  if (state.id === newId) {
    return state;
  }
