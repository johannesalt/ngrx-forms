...
const INITIAL_STATE = createFormControlState<string>('test ID', RADIO_OPTIONS[1]);
...
export class RadioTestComponent {
  ...
  public readonly options = input(RADIO_OPTIONS);
}
...
describe(RadioTestComponent.name, () => {
  ...
  it(`should trigger ${SetValueAction.name}s and ${MarkAsDirtyAction.name}s when switching between options`, () => {
    ...
    fixture.componentRef.setInput('state', { ...INITIAL_STATE, value: RADIO_OPTIONS[0] });
    fixture.detectChanges();
  ...
  it(`should trigger a ${SetValueAction.name} if the value of the selected option changes`, () => {
    ...
    fixture.componentRef.setInput('options', [RADIO_OPTIONS[0], newValue]);
    fixture.detectChanges();
